...
export default function Home() {
  ...
  useEffect(() => { if (publicKey && anchorWallet) checkVault(); }, [publicKey, anchorWallet]);

  const navItems = [
    { section: 'Accounts', items: [['Overview','/'],['Vault','/vault'],['Send privately','/send']] },
    { section: 'Private DeFi', items: [['Auctions','/auctions'],['Lending','/lending'],['Dark pool','/darkpool']] },
    { section: 'Agentic', items: [['AI agents','/x402'],['x402 gateway','/x402'],['A2A channels','/a2a']] },
    { section: 'Enterprise', items: [['Treasury','/treasury'],['Compliance','/vault'],['RWA rails','/vault']] },
  ];
          ...
          {navItems.map(({ section, items }) => (
            <div key={section} style={{ marginBottom:'28px' }}>
              ...
              {items.map(([label, href]) => (
                <Link key={label} href={href} onClick={() => setActiveNav(label)}
                  style={{ display:'flex', alignItems:'center', gap:'10px', padding:'10px 28px', color: activeNav===label ? '#c9a96e' : '#7a7468', borderLeft: activeNav===label ? '2px solid #c9a96e' : '2px solid transparent', background: activeNav===label ? 'rgba(201,169,110,0.1)' : 'transparent', fontSize:'13px', textDecoration:'none' }}>
                  <div style={{ width:'6px', height:'6px', borderRadius:'50%', border:'1px solid currentColor', background: activeNav===label ? '#c9a96e' : 'transparent' }}></div>
                  {label}
                </Link>
              ))}
            </div>
          ...
          {!connected ? (
            ...
            <>
              ...
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:'20px' }}>
                {[
                  ['⚡','Sealed auctions','Bid without revealing price. TEE-verified Vickrey.','/auctions'],
                  ['◈','Private lending','Borrow at confidential rates. Terms sealed in TEE.','/lending'],
                  ...
                  ['⟳','A2A channels','Agent-to-agent settlements inside PER.','/a2a'],
                  ['▦','Treasury','Corporate multi-sig vault. Shielded payroll.','/treasury'],
