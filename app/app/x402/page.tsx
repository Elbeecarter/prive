      ...
      <main style={{ flex: 1 }}>
        ...
        <div style={{ padding: '32px 36px' }}>
          <div style={{ ...card, borderColor: 'rgba(201,169,110,0.22)', background: 'rgba(201,169,110,0.04)', marginBottom: '20px' }}>
            <div style={{ fontSize: '13px', color: '#9a9488', lineHeight: '1.7' }}>⬡ <strong style={{ color: '#c9a96e' }}>Private x402 HTTP payments for AI agents.</strong> When an agent encounters HTTP 402 Payment Required, Privé routes the micropayment through MagicBlock PER. Agent identity and API relationships are hidden on-chain. Zero trace linking your agent to specific API calls.</div>
          </div>
            ...
            <>
              {phase === 'register' && (
                <div style={card}>
                  ...
                  <input style={input} value={agentName} onChange={e => setAgentName(e.target.value)} />
                  <span style={label}>Max payment per tx (USDC micro)</span>
                  <input style={input} value={maxPayment} onChange={e => setMaxPayment(e.target.value)} />
                  <span style={label}>Daily spending limit (USDC micro)</span>
                  <input style={input} value={dailyLimit} onChange={e => setDailyLimit(e.target.value)} />
              ...
              {(phase === 'pay' || phase === 'paid') && (
                <div style={card}>
                  <div style={{ fontFamily: 'Georgia, serif', fontSize: '18px', marginBottom: '6px' }}>
                    {phase === 'paid' ? '✓ Payment processed' : 'Simulate x402 API payment'}
                  </div>
                  <div style={{ fontSize: '12px', color: '#7a7468', marginBottom: '24px' }}>
                    Agent: {agentName} · Payments today: {payCount} · <a href={`https://explorer.solana.com/tx/${txSig}?cluster=devnet`} target="_blank" rel="noreferrer" style={{ color: '#c9a96e' }}>View registration tx →</a>
                  </div>
                  <div style={{ background: '#161616', border: '1px solid rgba(201,169,110,0.15)', borderRadius: '8px', padding: '16px', marginBottom: '16px', fontFamily: 'monospace', fontSize: '11px', color: '#9a9488', lineHeight: '1.8' }}>
                    ...
                    <span style={{ color: '#e05c5c' }}>HTTP 402 Payment Required</span><br />
                    <span style={{ color: '#c9a96e' }}>X-Payment-Required: {Number(paymentAmount) / 1_000_000} USDC</span><br />
                    {phase === 'paid' && <><span style={{ color: '#4caf7d' }}>✓ Payment sent via Privé PER</span><br /><span style={{ color: '#4caf7d' }}>✓ HTTP 200 OK · Resource unlocked</span></>}
                  </div>
                  <span style={label}>API endpoint</span>
                  <input style={input} value={apiEndpoint} onChange={e => setApiEndpoint(e.target.value)} />
                  <span style={label}>Payment amount (USDC micro)</span>
                  <input style={input} value={paymentAmount} onChange={e => setPaymentAmount(e.target.value)} />
